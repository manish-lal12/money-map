...
export const TaxDataProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  // General settings
  const [calculatorMode, setCalculatorMode] =
    useState<CalculatorMode>("simple");
  const [assessmentYear, setAssessmentYear] =
    useState<AssessmentYear>("2026-27");
  ...
  const [residentialStatus, setResidentialStatus] =
    useState<ResidentialStatus>("resident");
  ...
  const [employmentType, setEmploymentType] =
    useState<EmploymentType>("private");
  const [employerNpsContribution, setEmployerNpsContribution] =
    useState<number>(0);
  ...
  // Tax calculation results
  const [oldRegimeTaxableIncome, setOldRegimeTaxableIncome] =
    useState<number>(0);
  const [newRegimeTaxableIncome, setNewRegimeTaxableIncome] =
    useState<number>(0);
  ...
  const [oldRegimeMarginalRelief, setOldRegimeMarginalRelief] =
    useState<number>(0);
  const [newRegimeMarginalRelief, setNewRegimeMarginalRelief] =
    useState<number>(0);
  const [oldRegimeTotal, setOldRegimeTotal] = useState<number>(0);
  const [newRegimeTotal, setNewRegimeTotal] = useState<number>(0);
  const [oldRegimeFinalLiability, setOldRegimeFinalLiability] =
    useState<number>(0);
  const [newRegimeFinalLiability, setNewRegimeFinalLiability] =
    useState<number>(0);
  ...
  useEffect(() => {
    if (receivesHRA && rentPaid > 0) {
      const calculatedHraExemption = calculateHRAExemption(
        basicSalary,
        hraAmount,
        rentPaid,
        isMetroCity
  ...
  useEffect(() => {
    ...
    if (hasOtherIncome) {
      grossTotalIncome +=
        otherIncome + businessIncome + capitalGains + rentalIncome;
    }
    ...
    if (employerNpsContribution > 0) {
      const maxNpsLimit =
        employmentType === "private" ? basicSalary * 0.14 : basicSalary * 0.1;

      const eligibleEmployerNps = Math.min(
    ...
    const oldRebate = calculateRebate(
      oldBasicTax,
      taxableIncomeOldRegime,
      false
    );
    const newRebate = calculateRebate(
      newBasicTax,
    ...
    const newSurcharge = calculateSurcharge(
      newTaxAfterRebate,
      taxableIncomeNewRegime
    );
    ...
    const oldMarginalRelief = calculateMarginalRelief(
      oldTaxAfterRebate + oldSurcharge + oldCess,
      taxableIncomeOldRegime,
      false
    );
    const newMarginalRelief = calculateMarginalRelief(
      newTaxAfterRebate + newSurcharge + newCess,
      taxableIncomeNewRegime,
      true
    );
    ...
    const oldTotal = Math.max(
      0,
      oldTaxAfterRebate + oldSurcharge + oldCess - oldMarginalRelief
    );
    const newTotal = Math.max(
      0,
      newTaxAfterRebate + newSurcharge + newCess - newMarginalRelief
    );
  ...
  const getTaxRecommendations = () => {
    ...
    recommendations.push(
      `The ${betterRegime} tax regime is better for you, saving you ${formatCurrency(
        taxSavings
      )}`
    );
    ...
    if (betterRegime === "old") {
      if (section80C < 150000) {
        recommendations.push(
          `Invest ${formatCurrency(
            150000 - section80C
          )} more in 80C options like ELSS, PPF, or EPF to maximize your tax benefits`
        );
      }

      if (nps < 50000) {
        recommendations.push(
          `Consider investing ${formatCurrency(
            50000 - nps
          )} more in NPS to get additional tax benefits under 80CCD(1B)`
        );
      }

      if (section80D < 25000) {
        recommendations.push(
          `Get health insurance coverage to utilize remaining ${formatCurrency(
            25000 - section80D
      ...
      if (receivesHRA && rentPaid === 0) {
        recommendations.push(
          "Submit your rent receipts to claim HRA exemption and reduce your tax liability"
        );
      }

      if (hasHomeLoan && housingLoanInterest === 0) {
        recommendations.push(
          "Ensure you claim the full interest deduction on your home loan to reduce your taxable income"
        );
      }
    } else {
      recommendations.push(
        "Since you're in the new tax regime with fewer deductions, focus on maximizing your take-home salary rather than tax-saving investments"
      );

      if (employerNpsContribution === 0 && salaryIncome > 500000) {
        recommendations.push(
          "Consider requesting your employer to restructure your salary to include NPS contributions, as this is deductible even in the new regime"
        );
      }
    }
    ...
    if (tdsPaid === 0 && totalIncome > 500000) {
      recommendations.push(
        "Make sure to report all TDS deductions as this directly reduces your final tax payment"
      );
    }
